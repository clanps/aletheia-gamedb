...
### Translations
Translations are supported through gettext `.po` files, a template file can be generated by running `just generate_translations` and existing translation files can be updated with `just update_translations`. Translations should be stored in `ui/locale/<locale>/LC_MESSAGES/aletheia.po`.
